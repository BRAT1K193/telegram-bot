...
ADMIN_MODE = False
ADMIN_USERNAMES = ["@coobaalt"]  # Твой юзернейм
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args:
            ...
            if await check_subscription(user_id, context):
                ...
                for channel in CHANNELS:
                    try:
                        member = await context.bot.get_chat_member(channel, user_id)
                        if member.status not in ['member', 'administrator', 'creator']:
                            buttons.append([InlineKeyboardButton(f"📢 Подписаться на {channel}",
        ...
        # Просто игнорируем команду /start без аргументов
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.text.startswith('http') or 'loadstring(game:HttpGet(' in update.message.text:
        ...
        try:
            cursor.execute("INSERT INTO links (original_url, short_code) VALUES (?, ?)", (original_url, short_code))
            conn.commit()
